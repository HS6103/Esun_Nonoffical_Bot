...
if CHATBOT_MODE:
    try:
        responseDICT = json.load(open(os.path.join(os.path.dirname(os.path.dirname(__file__)), "reply\\reply_customer_service.json"), encoding="utf-8"))
    except Exception as e:
...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern=""):
    ...
    if utterance == "玉山電話銀行台幣綜合存轉[定存]服務相關規定":
        if CHATBOT_MODE:
            if args[1] in ['定存', '定期存款']:
