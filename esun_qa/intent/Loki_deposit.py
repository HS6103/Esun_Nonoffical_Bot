...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern=""):
    ...
    if utterance == "[下午6點]至[網路銀行]買/賣[外幣交易][額度]是算在[明天]嗎":
        if CHATBOT_MODE:
    ...
    if utterance == "[下午6點]至[網路銀行]買賣[外幣交易][額度]是算在[明天]嗎":
        if CHATBOT_MODE:
            resultDICT["response"] = getResponse(utterance, args)
        else:
            # write your code here
    ...
    if utterance == "[外]來[人口]換發「[新式統一證號]」[資料]變更提醒[事項]":
        if CHATBOT_MODE:
            resultDICT["response"] = getResponse(utterance, args)
        else:
            # write your code here
    ...
    if utterance == "[晚上8點]至[ATM][存款]到[玉山][帳戶][會]從[當日]計算[利息]嗎":
        if CHATBOT_MODE:
            resultDICT["response"] = getResponse(utterance, args)
        else:
            # write your code here
    ...
    if utterance == "[營業日][下午6點]使用[PayPal]提領入[新][臺幣][帳戶]之[交易][額度]是算於[明日]嗎":
        if CHATBOT_MODE:
            resultDICT["response"] = getResponse(utterance, args)
        else:
            # write your code here
    ...
    if utterance == "[金融卡]在[國外]提款[手續費]為何":
        if CHATBOT_MODE:
            resultDICT["response"] = getResponse(utterance, args)
        else:
            # write your code here
    ...
    if utterance == "[金融卡]在[國外]提款[每日][限額]及[手續費]為何":
        if CHATBOT_MODE:
            resultDICT["response"] = getResponse(utterance, args)
        else:
            # write your code here
    ...
    if utterance == "如何辦理[臺幣][一般][存款帳][戶]開戶":
        if CHATBOT_MODE:
            resultDICT["response"] = getResponse(utterance, args)
        else:
            # write your code here
    ...
    if utterance == "沒[時間]到[分行]開戶[是]否[可]委託[他人]辦理":
        if CHATBOT_MODE:
            resultDICT["response"] = getResponse(utterance, args)
        else:
            # write your code here
