...
if CHATBOT_MODE:
    try:
        responseDICT = json.load(open(os.path.join(os.path.dirname(os.path.dirname(__file__)), "reply/reply_wealth.json"), encoding="utf-8"))
    except Exception as e:
...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern=""):
    ...
    if utterance == "投信[基金]及[境][外][基金]淨值適用[說明]":
        if CHATBOT_MODE:
            resultDICT["response"] = getResponse(utterance, args)
        else:
            # write your code here
